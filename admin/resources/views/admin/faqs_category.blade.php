    ...
    <div class="col-md-12">
        <div class="card">
            <div class="header" style="padding-bottom: 10px;">
                <h2>カテゴリ一覧</h2>
                <button type="button" class="btn btn-primary mt-4" id="add-new-category">新たに追加</button>
            </div>
            <div class="body">
                <div class="table-responsive">
                    <table class="table center-aligned-table text-xs" id="category_list">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>カテゴリー名</th>
        ...
        <div class="modal-content">
            <form method="POST" action="" id="category_form">
                ...
                <div class="modal-header">
                    <h4 class="title" id="defaultModalLabel">Faq カテゴリ</h4>
                </div>
                                ...
                                <table class="text-xs w-full add-new-table" id="bot_form_data">
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input class="form-control w-full" name="ORDER" id="ORDER" data-validation="required" />
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
...
function remove(id) {
    swal({
        ...
        params['id'] = id;
        sendRequest("{{ url('/faqs/category/remove') }}", params, $('#category_list'), 2)
        .then((result) => {
            swal("削除!", "Faq カテゴリが削除されました。", "success");
            getData(0);
        })
        .catch(() => {
        });
...
function edit(id) {
    ...
    params['_token'] = "{{ csrf_token() }}";
    params['id'] = id;
    sendRequest("{{ url('/faqs/category/detail') }}", params, $('#add-new-modal'), 2)
    .then((result) => {
...
$('#category_form').on('submit', function(e) {
    ...
    sendRequest("{{ url('/faqs/category/create') }}", params, $('#add-new-modal'), 2)
    .then((result) => {
        $('#add-new-modal').modal('hide');
        swal("成功!", "Faq カテゴリが正常に保存されました。", "success");
        getData(0);
        $('#NAME').val('');
        $('#ORDER').val('');
        $('#data_id').val('');
...
function getData(lastID) {
    ...
    params['id'] = lastID;
    $.ajax(  { url: "{{ url('') }}/faqs/category/get_list",
            type: 'POST',
            data: params,
            beforeSend: function() {
                $('.load-more').show();
